...
def generate_crossPhantom(dataset, **kwargs): # use noise_level
    ...
    noise_level = kwargs['noise_level'] if ('noise_level' in kwargs) else 0
    if ('noise_level' in kwargs) and (noise_level != 0):
        display_delta =True
    else:
        display_delta = False
    assert dataset in [15,60]
...
def generate_stempo1(data_set = 'real', data_thinning = 2,views=11, new_shape= (64,64),columnsshift=14, **kwargs):
        """
        Generate stempo observations
        """
        nt = kwargs['nt'] if ('nt' in kwargs) else 10
...
def generate_emoji2(dataset, n, **kwargs):
    ...
    [mm, nn] = sinogram.shape
    ind = []
    for ii in range(int(nn /n)):
        ind.extend( np.arange(0,mm) + (n*ii)*mm )
    m2 = sinogram[:, 0::n]
    A_small = A[ind, :]
    b = m2
    nt = int(T)
    ...
    # delta = 0 # np.linalg.norm() # no added noise for this dataset, change to allow added noise.
    for ii in range(T):
        AA[ii] = A_small[n2*(ii):n2*(ii+1), 16384*ii:16384*(ii+1) ]
        B[ii] = b[n2*(ii) : n2*(ii+1)]
    if display_delta == False:
        return (A_small, b, AA, B, nx, ny, nt)
    else:
        return (A_small, b, AA, B, nx, ny, nt, delta)