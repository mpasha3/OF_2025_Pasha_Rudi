...
def gen_spacetime_derivative_operator(nx, ny, nt):
    ...
    Lt = gen_first_derivative_operator(nt)
    ITLs = sparse.kron(sparse.identity(nt), D_spatial)
    LTIN = sparse.kron(Lt, sparse.identity(nx*ny))
    L =  sparse.vstack((ITLs, LTIN))
    return L 
...
class OffsetFirstDerivative(LinearOperator):
    def __init__(self, n, delta_x_steps=1):
        """Initialize the first derivative operator with discrete offsets.
        
        Parameters:
        n (int): Length of the input array.
        delta_x_steps (int): Number of indices to offset for finite difference.
        """
        super().__init__(dtype="float64", shape=(n, n))
        self.n = n
        self.delta_x_steps = delta_x_steps

    def _matvec(self, u):
        """Forward operation for the first derivative with discrete offset."""
        du_dx = np.zeros_like(u)

        # Apply centered difference with discrete offset for interior points
        du_dx[self.delta_x_steps:-self.delta_x_steps] = (
            u[self.delta_x_steps*2:] - u[:-self.delta_x_steps*2]
        ) / (2 * self.delta_x_steps)
        
        # Boundary handling
        du_dx[:self.delta_x_steps] = (u[self.delta_x_steps:2*self.delta_x_steps] - u[:self.delta_x_steps]) / self.delta_x_steps
        du_dx[-self.delta_x_steps:] = (u[-self.delta_x_steps:] - u[-2*self.delta_x_steps:-self.delta_x_steps]) / self.delta_x_steps

        return du_dx

    def _rmatvec(self, u):
        """Adjoint operation for the first derivative (optional for completeness)."""
        return self._matvec(u)
def first_derivative_operator_2d_offset(nx, ny,v_max):

    D_x = OffsetFirstDerivative(nx,v_max)
    D_y = OffsetFirstDerivative(ny,v_max)
