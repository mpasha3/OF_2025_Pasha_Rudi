...
def gen_im_seq_(shape,t_end=3,v_max = 2,v_min = 1):
    v_mag_= v_min
    v_mag = v_max
    us = []
...
def gen_joint_tomo_op_and_data(x_traj, t_end, nx,ny, views,noise_level=1e-2,case='a'):
    np.random.seed(0)
    ...
    if (case == 'a'):
        ...
        for x_true in x_traj:
            ...
            (A, b_true, p, q, AforMatrixOperation) = Tomo.gen_data(x_true, nx, ny, views,min_angle,max_angle)
            As.append(A)
    ...
    b_traj_and_deltas = [Tomo.add_noise(b_true, opt = 'Gaussian', noise_level=noise_level) for b_true in b_trues]
    ...
    return (X_bar, A_bar,data_vec_bar,data_vec_true_bar,b_shape,L_bar,I_bar,deltas)


def gen_joint_blur_op_and_data(x_traj, t_end, nx,ny, spread,noise_level=1e-2):
    A = build_A(spread=spread, shape = (nx,ny))
    ...
    for x_true in x_traj:
        ...
        b_trues.append(b_true)
        j+=1
    b_traj_and_deltas = [add_noise(b_true, opt = 'Gaussian', noise_level=noise_level) for b_true in b_trues]
    b_traj = np.array(b_traj_and_deltas,dtype=object)[:,0]
    deltas = np.array(b_traj_and_deltas,dtype=object)[:,1]
    delta = deltas[0]
    L = gen_first_derivative_operator_2D(nx, ny)
...
def gen_joint_blur_op_and_data2(x_traj, t_end, nx,ny, spread,dim= (1,1),noise_level=1e-2):
